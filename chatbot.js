<!-- ========================================================================================= -->
<!-- chatbot.js (SCRIPT EXEMPLO) - Versão completa, exceto base de conhecimento resumida       -->
<!-- ========================================================================================= -->
<script>
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Definição das mensagens de saudação para cada idioma
  const greetings = {
    pt: {
      morning: "Bom dia",
      afternoon: "Boa tarde",
      evening: "Boa noite",
      help: "Como podemos ajudar? Qual a sua dúvida?"
    },
    en: {
      morning: "Good morning",
      afternoon: "Good afternoon",
      evening: "Good evening",
      help: "How can we help? What is your question?"
    },
    es: {
      morning: "Buenos días",
      afternoon: "Buenas tardes",
      evening: "Buenas noches",
      help: "¿Cómo podemos ayudar? ¿Cuál es tu duda?"
    }
  };

  const now = new Date();
  const hour = now.getHours();
  let greetingText = greetings[lang].morning;
  if (hour < 12) {
    greetingText = greetings[lang].morning;
  } else if (hour < 18) {
    greetingText = greetings[lang].afternoon;
  } else {
    greetingText = greetings[lang].evening;
  }

  // Mensagem de boas-vindas disparada imediatamente
  const welcomeMessage = `${greetingText}! ${greetings[lang].help}`;
  addMessage(welcomeMessage, "bot");

  // Placeholder do input de acordo com o idioma
  const placeholderTexts = {
    pt: "Digite sua mensagem...",
    en: "Type your message...",
    es: "Escribe tu mensaje..."
  };
  document.getElementById("message-input").placeholder = placeholderTexts[lang];

  // --------------------------------------------------------
  // 2. INCLUSÃO DAS BASES DE CONHECIMENTO
  // --------------------------------------------------------

  // ========================================================
  // Base de Conhecimento Interna (knowledgeBase)
  // ========================================================
  // (ATENÇÃO: A estrutura abaixo contém diversas declarações de variáveis e arrays referentes a diferentes áreas.
  // Caso sua intenção seja utilizar uma única base de conhecimento para o chatbot, certifique-se de que as informações
  // estejam consolidadas na variável "knowledgeBase".)
...
const logistica = [
  ...
  // A lista continua – inclua os demais itens conforme o conjunto de URLs fornecido.
];
...
const custos = [
  ...
  },
  {
    ...
    url: "https://helpsinge.lince.com.br/relacao_conta_centro_de_custos_.html?ms=AwACAABAAEAAAAAIQAgQCA%3D%3D&st=MA%3D%3D&sct=MTQ4&mw=MzAw",
    content: "Permite a associação entre contas contábeis e centros de custos, facilitando o controle e a alocação dos gastos por departamento ou área."
  },
  {
    id: "simulacao_rentabilidade_filial",
    title: "Simulação de Rentabilidade por Filial",
    url: "https://helpsinge.lince.com.br/_simulacao_de_rentabilidade_por_filial.html?ms=AwACAABAAEAAAAAIQAgQCA%3D%3D&st=MA%3D%3D&sct=MTQ4&mw=MzAw",
  ...
  },
  {
    ...
    url: "https://helpsinge.lince.com.br/valores_para_simulacao_de_precos.html?ms=AwACAABAAEAAAAAIQAgQCA%3D%3D&st=MA%3D%3D&sct=MTQ4&mw=MzAw",
  ...
  },
  {
    ...
    url: "https://helpsinge.lince.com.br/valores_para_custos.html?ms=AwACAABAAEAAAAAIQAgQCA%3D%3D&st=MA%3D%3D&sct=MTQ4&mw=MzAw",
...
const knowledgeBaseMixed = [  
  ...
  // Exemplo de função para “achatar” se tiver arrays aninhados:
  function flattenKnowledgeBase(kbArray) {
    return kbArray.reduce((acc, item) => {
  ...
  // Se você quiser atualizar dinamicamente o conteúdo da base:
  async function updateKnowledgeBaseWithLiveContent(knowledgeBase) {
    for (const item of knowledgeBase) {
      if (item.url) {
        const liveText = await fetchPageText(item.url);
        if (liveText && liveText.length > 0) {
          item.liveContent = liveText;
          const processed = preprocessText(liveText);
          const sentences = processed
            .split(/[.!?]+/)
            .map(s => s.trim())
            .filter(s => s.length > 0);
          item.sentences = sentences;
          console.log(`Conteúdo atualizado para "${item.title}"`);
        }
      }
    }
  }
  ...
  function calculateSimilarity(text, query) {
    ...
    const similarity = matches / queryWords.size;

    // Pequena penalidade se texto for muito grande (exemplo):
    const wordCount = text.split(/\s+/).length;
    const lengthPenalty = wordCount > 30 ? 30 / wordCount : 1;
  ...
  async function findBestAnswer(query) {
    ...
    flattenedKnowledgeBase.forEach(doc => {
      const titleSimilarity = calculateSimilarity(preprocessText(doc.title), cleanedQuery);

      // Se não houver sentenças, use o content todo
      const sentences = doc.sentences && doc.sentences.length > 0
        ? doc.sentences
        : [preprocessText(doc.content || doc.title)];

      sentences.forEach(sentence => {
        const sentenceSimilarity = calculateSimilarity(sentence, cleanedQuery);
        const totalScore = 0.4 * titleSimilarity + 0.6 * sentenceSimilarity;

        if (totalScore > bestScore) {
          bestScore = totalScore;
          bestMatch = { doc, sentence, score: totalScore };
        }
      });
    });
  ...
  async function handleUserMessage(message) {
    try {
      ...
      if (bestMatch && bestMatch.score > 0.1) {
        const snippet = bestMatch.sentence || "";
        const answer = `
          Encontrei esta informação relevante:<br><br>
          "${snippet}"<br><br>
          <strong>${bestMatch.doc.title}</strong><br>
          <a href="${bestMatch.doc.url}" target="_blank">Abrir link</a>
        `;
        addMessage(answer, "bot");
      } else {
        let fallback = "Não encontrei uma resposta exata. Confira alguns documentos:<br>";
        flattenedKnowledgeBase.slice(0, 3).forEach((kb, idx) => {
          fallback += `${idx + 1}. <strong><a href="${kb.url}" target="_blank">${kb.title}</a></strong><br>`;
        });
        addMessage(fallback, "bot");
      }
    } catch (error) {
      console.error("Erro no processamento:", error);
      addMessage("Ocorreu um erro ao processar sua solicitação. Tente novamente mais tarde.", "bot");
    }
  }
  ...
  if (!chatToggle || !chatWidget || !chatClose || !chatMinimize || !messageInput || !sendButton || !chatMessages) {
    console.error("Elementos do chatbot não foram encontrados no DOM. Verifique os IDs no HTML.");
    return;
  } else {
  ...
  function toggleMinimizeChat() {
    // Ajuste do ícone para '☰' no estado minimizado
    if (chatWidget.classList.contains("minimized")) {
      console.log("Restaurando o chat (desminimizando)...");
      chatWidget.classList.remove("minimized");
      chatMinimize.textContent = '_';
      messageInput.focus();
    } else {
  ...
  chatClose.addEventListener("click", function(e) {
    console.log("Clique no chat-close detectado.");
    closeChat();
  });
  chatMinimize.addEventListener("click", function(e) {
    console.log("Clique no chat-minimize detectado.");
    toggleMinimizeChat();
  });
  ...
  window.addEventListener("scroll", function () {
    ...
    if (footer && chatToggle) {
      const footerRect = footer.getBoundingClientRect();
      const windowHeight = window.innerHeight;
      if (footerRect.top < windowHeight) {
        // Calcula o quanto o footer está visível
        const overlap = windowHeight - footerRect.top;
        chatToggle.style.bottom = (defaultChatToggleBottom + overlap) + "px";
      } else {
        chatToggle.style.bottom = defaultChatToggleBottom + "px";
      }
    }
  });
});
